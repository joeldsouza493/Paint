...
int main(int argc, char *argv[]) {
	...
	//initialize window
	window = SDL_CreateWindow("Paint", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 1366, 768, SDL_WINDOW_SHOWN);
        if(window == NULL){
	...
	screen = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA8888, SDL_RENDERER_TARGETTEXTURE, 1366, 768);
...
int drawLine(SDL_Renderer *renderer, SDL_Rect dst, SDL_Texture *screen) {
	...
	SDL_Surface *s = SDL_CreateRGBSurface(0, 1366, 768, 32, 0, 0, 0, 0);
	
	SDL_RenderReadPixels(renderer, NULL, 0, pix, 1366 * 4);
	s->pixels = pix;
...
int drawHand(SDL_Renderer *renderer, SDL_Rect dst) {
	...
	while(1) {
		if(SDL_PollEvent(&ev)) {
			...
			else if(ev.type == SDL_MOUSEMOTION && x1 > CANVAS_X && flag == 1) {
				SDL_GetMouseState(&x2, &y2);	
				SDL_SetRenderDrawColor(renderer, Color.r, Color.g, Color.b, 255);
				SDL_RenderDrawLine(renderer, x1, y1, x2, y2);
				x1 = x2;
	...
	while(1) {
		if(SDL_PollEvent(&ev)) {
			...
			if(ev.type == SDL_MOUSEBUTTONDOWN) {
				...
				SDL_SetRenderDrawColor(renderer, Color.r, Color.g, Color.b, 255);
				myEllipse(renderer, x1, y1, x2, y2);
				SDL_RenderPresent(renderer);
	...
	while(1) {
		if(SDL_PollEvent(&ev)) {
			...
			if(ev.type == SDL_MOUSEBUTTONDOWN) {
				...
				SDL_RenderReadPixels(renderer, &rect, 0, &pixels, 1366*4);
				p = (Uint8 *)&pixels;
...
//-------------------------------------------------------
	...
	cscreen = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA8888, SDL_RENDERER_TARGETTEXTURE, 440, 280);
		...
		if(SDL_PollEvent(&ev)) {
			if(ev.type == SDL_QUIT)
				...
				if(x > CUSTOM1_X && x < CUSTOM1_X + CUSTOM1_W && y > CUSTOM1_Y && y < CUSTOM1_Y + CUSTOM1_H + 1) {
					...
					SDL_RenderReadPixels(r, &rect, 0, &pixels, 1366*4);
					p = (Uint8 *)&pixels;
	...
	while(1) {
		if(SDL_PollEvent(&ev)) {
			if(ev.type == SDL_QUIT)
				...
				rect.x = x;
				rect.y = y;
				SDL_RenderReadPixels(renderer, &rect, 0, &p, 1366 * 4);
				SDL_RenderReadPixels(renderer, NULL, 0, pix, 1366 * 4);
				SDL_SetRenderDrawColor(renderer, Color.r, Color.g, Color.b, 255);
...
void floodFill(SDL_Renderer *renderer, int x, int y, Uint32 p) {
	...
	if(((x + y * 1366) < 0) || ((x + y * 1366) >= 1366 * 768))
		return;
	...
	pix[x + y * 1366] = (Color.b << 24) | (Color.g << 16) | (Color.r << 8);
...
int saveTexture(SDL_Renderer *renderer, SDL_Rect dst) {
	...
	sprintf(filepath, "./Saves/image%d-%d-%d_%d:%d:%d.png", info->tm_mday, info->tm_mon, info->tm_year % 100, info->tm_hour, info->tm_min, info->tm_sec);
...
void myEllipse(SDL_Renderer *renderer, int x1, int y1, int x2, int y2) {
	...
	rect.w = 1;
	rect.h = 1;
	float a = (x2 - x1) / 2, b = (y2 - y1) / 2, i, h = (x1 + x2) / 2, k = (y1 + y2) / 2, x, y, tx, ty;
	tx = h + a;
	ty = k;
	float d = (22 / 7.0) / 360.0;	
	for(i = 0; i < 360; i++) {
		x = h + a * cos(d * i);
		y = k + b * sin(d * i);
		rect.x = x;
...
void push(stack *s, SDL_Renderer *renderer) {
        ...
        SDL_RenderReadPixels(renderer, NULL, 0, n->pix, 1366 * 4);
        n->next = s->start;
        s->start = n;
        (s->len)++;
}
...
Uint32 *pop(stack *s) {
        node *temp;
        int i; 
        Uint32 *pix = (Uint32 *)malloc(1366 * 768 * sizeof(Uint32));
