...
int main(int argc, char *argv[]) {
	...
	canvas = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA8888, SDL_RENDERER_TARGETTEXTURE, 1280, 960);
		...
		if(SDL_PollEvent(&ev)) {
			if(ev.type == SDL_QUIT) {
				...
				if(x > LINE_X && x < LINE_X + LINE_W && y > LINE_Y && y < LINE_Y + LINE_Y) {
					if(!drawLine(renderer, dst[line], canvas))
						break;
				}
					...
					if(!drawRectangle(renderer, dst[rectangle], canvas))
						break;
				}
...
/*				else if(x > dst[ellipse].x && x < dst[ellipse].x + dst[ellipse].w && y > dst[ellipse].y && y < dst[ellipse].y + dst[ellipse].h ) {
					...
					if(!drawPolygon(renderer, dst[polygon], canvas))
						break;
				}
					...
					if(!getColor(renderer, dst[getcolor], dst[palettetool], canvas))
						break;
				}
...
//-------------------------------------------------------
	...
	SDL_Window *win = NULL;
	SDL_Renderer *r = NULL;
	SDL_Texture *csurface = NULL;

	win = SDL_CreateWindow("Color Palette", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 320, 240, SDL_WINDOW_SHOWN);
        if(win == NULL){
                printf("Could not create Window: %s\n", SDL_GetError());
                exit(1);
        }

	csurface = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA8888, SDL_RENDERER_TARGETTEXTURE, 1280, 960);
	r = SDL_CreateRenderer(win, -1, SDL_RENDERER_ACCELERATED);
	SDL_SetRenderTarget(r, csurface);
